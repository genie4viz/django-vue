...
BEGIN
    ...
    IF ST_Length2D(ST_GeomFromEWKT(geom),true) > 0 THEN
      result.slope := (result.max_elevation - result.min_elevation) / ST_Length2D(ST_GeomFromEWKT(geom),true);
    END IF;
...
BEGIN
    ...
    ELSE
        ...
        FOR t_offset, t_geom IN SELECT e.e_offset, ST_Smart_Line_Substring(t.shape_2d, et.start_position, et.end_position)
               FROM hike_event e, hike_eventtrailsection et, hike_trailsection t
               WHERE e.event_id = eid AND et.evnt = e.event_id AND et.trailsection = t.trailsection_id
    ...
    IF t_count > 0 THEN
        -- We remove 3D calculation into Django code for performance
